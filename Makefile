CXX = g++
CXXFLAGS  = -Xcompiler -Wall -Xcompiler -fopenmp -D USE_OPENMP
...
NVFLAGS = --expt-relaxed-constexpr --gpu-architecture sm_70 -Xptxas -O3,-v -allow-unsupported-compiler -use_fast_math

cbet-gpu: def_f.cuh main.cu launch_ray_XZ.cu multi_gpu.cpp
	$(NVCC) $(NVFLAGS) $(CXXFLAGS) -o cbet-gpu main.cu launch_ray_XZ.cu multi_gpu.cpp $(LDFLAGS)

double: def_d.cuh main.cu launch_ray_XZ.cu multi_gpu.cpp
	$(NVCC) $(NVFLAGS) $(CXXFLAGS) -D DOUBLE -o cbet-gpu main.cu launch_ray_XZ.cu multi_gpu.cpp $(LDFLAGS)

test: def_f.cuh main.cu launch_ray_XZ.cu
